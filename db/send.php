...
// >>> Cliente Interno
    ...
    //$mail->addAddress('contacto@decoraespejos.cl');
    $mail->addAddress('spizarro@sociodigital.cl');
	
	$mail->isHTML(true);
    $mail->Subject = 'Deco Espejos has recibido un NUEVO CONTACTO';
	
    $message = '<html><body>';
	$message .= "<p>Hola Deco Espejos, gracias a la campaña de publicidad digital que actualmente tienes online has recibido un nuevo contacto interesado en tus servicios:</p>";
	...
	$message .= "<tr><td><strong>Página de captura:</strong> </td><td>" . strip_tags($servicio) . "</td></tr>";
	$message .= "<tr><td><strong>¿Qué deseas cotizar?:</strong> </td><td>" . strip_tags($mensaje) . "</td></tr>";
...
// >>> Cliente Externo	
	...
	$mail2->setFrom('notificaciones@decoraespejos.cl', 'Deco Espejos');
    $mail2->addAddress($correo);
    $mail2->addReplyTo('contacto@decoraespejos.cl');
	
	$mail2->isHTML(true);
    $mail2->Subject = 'Hemos recibido tu solicitud exitosamente';
	
	$message = '<html><body>';
	$message .= "<p>Hola " . strip_tags($nombre) . ", gracias por confiar en nosotros y haber solicitado una cotización.</p>";

    if(date('N')<=5){
			
        if(date('G')<18){ $message .= "<p>Durante la jornada nos contactaremos contigo para enviarte la cotización solicitada. ¡Hablamos pronto!.</p>";}
        else{ $message .= "<p>Lamentablemente, debido a que ya terminamos nuestra jornada de cotizaciones, no estamos atendiendo solicitudes en este preciso momento, pero te contactaremos el próximo día hábil a primera hora sin falta.<p>";}
        
    }else{ 
    
        $message .= "<p>Lamentablemente, debido a que es fin de semana, no estamos atendiendo cotizaciones en este preciso momento, pero te contáctaremos el próximo día hábil sin falta.</p>";
    
    }

	$message .= "<p>Si deseas contactar inmediatamente con nosotros, dispones de los siguientes métodos de comunicación:</p>";
	$message .= "<ul>";
	$message .= "<li>Llámanos o escríbenos un whatsapp al número +56932490310.</li>";
	$message .= "<li>Envíanos un correo directamente a contacto@decoraespejos.cl o respondiendo este mail.</li>";
	$message .= "</ul>";
